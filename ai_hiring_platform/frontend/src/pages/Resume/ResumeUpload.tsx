...
export const ResumeUpload: React.FC = () => {
  ...
  const handleUpload = async () => {
    ...
    if (pendingFiles.length === 1) {
      // Single file upload — use the original endpoint
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      <PageHeader
        title="Resumes"
        description="Upload candidate resumes (PDF or DOCX). Select multiple files or drag a folder to bulk-upload."
      />
      ...
      <div className="max-w-3xl space-y-6">
        ...
        {/* Bulk upload outcome. When 100+ resumes go up at once the only thing that
            matters is which ones did NOT land, so failures are named explicitly. */}
        {bulkResult && (
          <div className="rounded-xl border border-white/10 bg-card p-4">
            <div className="flex flex-wrap items-center gap-3">
              ...
              {/* Duplicates are counted apart from failures: re-sending a CV already
                  in the pool is expected in a large batch, and burying it among real
                  errors hides the errors that actually need attention. */}
              {(bulkResult.duplicate_count ?? 0) > 0 && (
                <span className="inline-flex items-center gap-1.5 text-sm font-semibold text-amber-400">
                  <AlertCircle className="h-4 w-4" />
                  {bulkResult.duplicate_count} already in the pool
                </span>
              )}
                      ...
                      <span className="min-w-0">
                        ...
                        {r.duplicate ? (
                          <span className="text-gray-500">
                            {' '}— already uploaded as{' '}
                            <span className="text-amber-300/80">{r.existing_filename}</span>
                            {r.existing_id !== undefined && ` (#${r.existing_id})`}
                          </span>
                        ) : (
